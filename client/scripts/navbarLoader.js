...
<nav class="user-navbar">
  ...
  eBay<span class="bd-sub">BD</span>
</a>
...
/* BD = 1/3 of eBay font size */
.logo .bd-sub {
  font-size: calc(1em / 3);  /* EXACT one-third */
  position: relative;
  top: 0.35em;
  margin-left: 3px;
  font-weight: 900;
...
.user-navbar {
 ...
 min-height: 35px; /* instead of fixed 30px */
  ...
  background:rgba(194, 94, 94, 0.93);
  color: white;
...
.navbar-search {
  display: flex;
  align-items: center;
  background:  rgba(178, 212, 243, 0.96);
  border: 2px solid rgba(230, 236, 228, 0.11);
  border-radius: 20px;
  overflow: hidden;
  height: 38px; 
}
...
.user-navbar .navbar-search button {
  ...
  padding: 0 0.75rem;
  font-size: 1rem;
  color: black; /* white button to click */
  height: 100%;
  cursor: pointer;
}
...
.nav-username:hover, .nav-user-dropdown.open .nav-username {
  background: rgba(242, 194, 243, 0.18);
}
.dropdown-menu {
  position: absolute;
  top: 110%;
  ...
  color: #132721ff;
  border-radius: 8px;
  box-shadow: 0 4px 16px rgba(45, 198, 96, 0.13);
  z-index: 3000;
  padding: 0.3em 0;
  display: none;
}
