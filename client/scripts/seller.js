...
document.getElementById('upload-product-form').addEventListener('submit', async (e) => {
  ...
  if (!name || isNaN(price) || price < 0 || isNaN(quantity) || quantity < 0 || !description) {
    showFormMessage('Please fill all required fields (name, description, price, quantity).', true);
    return;
  }
  ...
  try {
    const uploadUrl = API_BASE + '/api/products';
    console.log('Uploading product to:', uploadUrl);
    ...
    if (res.ok) {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        errorMessage = 'Server error (HTTP ' + res.status + '): Check console for details';
      }
      
      console.error('Upload failed:', res.status, errorMessage);
      showFormMessage('Failed to upload product: ' + errorMessage, true);
    ...
    showPopupMessage('Network error: ' + err.message, false);
  }
  ...
  try {
    ...
    if (summaryDiv) {
      ...
      if (graphCanvas && approvedProducts.length > 0) {
        ...
        const topPad = Math.floor(canvasHeight * 0.13); 
        const bottomPad = Math.floor(canvasHeight * 0.18); 
  ...
  document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.onclick = function() {
      ...
      showDeleteConfirmation(async (confirmed) => {
        if (confirmed) {
          try {
            const res = await fetch(`${API_BASE}/api/products/${btn.dataset.id}`, {
              method: 'DELETE',
              headers: { 'Authorization': 'Bearer ' + token }
            });
            if (res.ok) {
