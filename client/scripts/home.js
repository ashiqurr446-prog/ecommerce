document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchProducts() {
    try {
      const res = await fetch(API_BASE + '/api/products?approvedOnly=true');
  ...
  function displayProducts() {
    ...
    productContainer.innerHTML = filtered.map(p => {
      ...
      if (reviewCount > 0) {
        ...
        starsHtml = '<span style="color:#333;">No ratings</span>';
      }
      return `
        <div class="product">
  ...
  // Auto-refresh products every 10 seconds to show newly approved items
  setInterval(() => {
